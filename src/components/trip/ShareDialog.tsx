...
export function ShareDialog({ isOpen, onClose, tripId, tripTitle }: ShareDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-4xl p-0 gap-0 overflow-hidden rounded-2xl sm:rounded-3xl border-none shadow-2xl">
        <div className="grid sm:grid-cols-5 h-full">
          ...
          <div className="sm:col-span-3 p-5 sm:p-8 flex flex-col gap-4 sm:gap-6 bg-white">
            {/* Header */}
            ...
            {/* Social Icons - Scrollable on Mobile, Grid on Desktop */}
            <div 
              className="flex sm:grid sm:grid-cols-6 gap-3 sm:gap-4 overflow-x-auto sm:overflow-visible pb-2 sm:pb-0 -mx-5 sm:mx-0 px-5 sm:px-0 my-2 sm:my-4"
              style={{ 
            ...
            >
              {socialApps.map((app) => (
                <button
                  ...
                  className="flex flex-col items-center gap-1.5 sm:gap-2 group transition-transform hover:scale-105 active:scale-95 flex-shrink-0 min-w-[56px] sm:min-w-0"
                >
            ...
            {/* Link Section - Different styles for Mobile vs Desktop */}
